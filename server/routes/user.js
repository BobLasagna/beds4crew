...
router.put("/toggle-role", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to toggle role", error: error.message });
  }
});

// Toggle hasPaid status
router.put("/toggle-payment", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to toggle payment status", error: error.message });
  }
});
